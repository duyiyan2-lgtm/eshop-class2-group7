...
索引：用户名唯一索引；`idx_sys_user_status(status)`。
...
- V1～V5 已在真实 MySQL 8.4 环境完成多轮回归。
- V6/V7 对应业务已通过 H2 集成测试和前端生产构建。
- 2026-07-30 因 Docker Desktop 未启动，本轮尚未重新执行 V6/V7 的真实 MySQL 迁移回归；验收前必须启动 Docker 后补跑并将结果写入测试报告。
